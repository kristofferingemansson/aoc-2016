...
func (b *Building) Tick() {
	...
	if found {
		if movesCount <= b.moves {
			if debug {
				fmt.Println("Dead end: Already found")
			}
			return
		}
	...
	if len(m) == 0 {
		if debug {
			fmt.Println("Dead end: No movables")
		}
		return
	}
...
func (b *Building) IsFinished() bool {
	for i, l := 0, len(b.floors) - 1; i < l; i++ {
