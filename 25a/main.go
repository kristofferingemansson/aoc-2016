...
func main() {
	...
	for x := 0; x < 1000000000; x++ {
		...
		instance.memory["a"] = Value(x)
		n := 0
		for instance.Next() {
			//fmt.Println(instance.ptr, instance.output)
			if !IsValidSignal(instance.output) {
				break
			}
			n++
			if n == 10000 {
			...
			if n == 1000000000 {
				fmt.Println(x)
				fmt.Println("Done!", x, instance.ptr, instance.output)
				break
			}
		}
	}
...
func GetInputData() []string {
	...
	input, err := ioutil.ReadFile(dir + "/25a/input.txt")
	if err != nil {
		log.Fatalln(err)
	}
...
func (i InstructionOut) Toggle() Instruction {
	return InstructionIncr{i.a}
}
...
func (i InstructionOut) Apply(inst *Instance) {
	value := i.a.value
	if i.a.register != "" {
		value = inst.memory[i.a.register]
	}
	inst.output = append(inst.output, int(value))
	inst.ptr++
}
...
func IsValidSignal(signal []int) bool {
	for i, bit := range signal {
		m := int(math.Mod(float64(i), 2))
		if m != bit {
			return false
		}
	}
